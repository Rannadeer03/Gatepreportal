...
const AcademicTeacherCourseUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    if (!materialType) { setError('Please select material type'); return; }
    try {
      setError('');
      ...
      setFile(null); setTitle(''); setDescription(''); setMaterialType(''); setSelectedSubject('');
  ...
  const handleDelete = async (materialId: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete course material. Please try again.');
    }
  };
      ...
      <div className="p-6 bg-white rounded-lg shadow-md mb-8">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-white font-medium ${file && selectedSubject && title.trim() && description.trim() && materialType ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'}`}
          >
            Upload Material
          </button>
        </div>
...
export default AcademicTeacherCourseUpload; 