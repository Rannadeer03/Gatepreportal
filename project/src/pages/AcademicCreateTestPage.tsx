...
import { Save, Plus, Trash2, ChevronLeft, ChevronRight, Image as ImageIcon, Type, Upload, CheckCircle, AlertTriangle } from 'lucide-react';
...
const AcademicCreateTestPage: React.FC = () => {
  ...
  const isTestValid = () => {
    const validations = {
      ...
      hasValidQuestion: questions.some(q => {
        ...
        const hasCorrectOption = q.correct_option !== '' && !isNaN(parseInt(q.correct_option)) && parseInt(q.correct_option) >= 0 && parseInt(q.correct_option) < q.options.length;
        const hasValidOptions = q.options.every(opt => opt.trim() !== '' || opt.startsWith('[IMG]'));
  ...
  useEffect(() => {}, [testTitle, subject, duration, questions]);
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      setError(null);
      // Create the test in academic_tests
      const { data: createdTest, error: testError } = await supabase
        ...
        .select()
        .single();
      if (testError || !createdTest) throw testError || new Error('Failed to create test');
      // Create questions in academic_questions
      ...
      for (const q of questions) {
        const { data: question, error: questionError } = await supabase
          ...
          .select()
          .single();
        if (questionError || !question) throw questionError || new Error('Failed to create question');
        createdQuestions.push(question);
      }
      // Add questions to academic_test_questions
      const testQuestions = createdQuestions.map((q, idx) => ({
      ...
      setError(error.message || 'An error occurred while creating the test');
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900">Create New Academic Test</h1>
            <button
...
export default AcademicCreateTestPage; 