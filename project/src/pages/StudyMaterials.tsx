      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-8">
        ...
        >
          ...
          <span className="text-sm font-medium">
            {selectedSubject ? 'Back to Subjects' : 'Back to Student Dashboard'}
          </span>
        </button>
        {/* Heading */}
        <h1 className="text-2xl font-extrabold text-gray-900 text-center mb-1">Study Materials</h1>
        ...
        {!selectedSubject && (
          <div className="bg-white rounded-xl shadow p-2 mb-6 drop-shadow-sm">
            <div className="relative">
              <Search className="absolute left-2 top-1/2 transform -translate-y-1/2 text-indigo-400 h-4 w-4" />
              <input
                type="text"
                placeholder="Search subjects..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-8 pr-2 py-1.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 text-sm bg-indigo-50 placeholder-indigo-300"
              />
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {materials.length === 0 ? (
              ...
              materials.map((material) => (
                <div
                  key={material.id}
                  className="bg-white rounded-xl shadow border border-gray-100 p-4 flex flex-col h-full justify-between hover:shadow-lg hover:border-indigo-200 transition-all duration-200"
                >
                  <div className="flex items-center mb-3">
                    <div className="p-2 bg-gradient-to-br from-indigo-400 to-blue-400 rounded-lg mr-2 shadow">
                  ...
                  <div className="flex items-center space-x-2 mt-auto">
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1.5 border border-indigo-200 rounded-lg text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 hover:border-indigo-400 transition"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {filteredSubjects.map((subject) => (
              <button
                ...
                className="bg-white rounded-xl shadow border border-gray-100 p-5 flex flex-col items-center cursor-pointer hover:shadow-lg hover:border-indigo-200 transition-all duration-200 group"
              >
                <div className="p-2.5 bg-gradient-to-br from-indigo-400 to-blue-400 rounded-lg mb-3 shadow">
                ...
                <div className="mt-2 flex items-center justify-center">
                  <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-indigo-100 group-hover:bg-indigo-400 transition">
                    <ChevronRight className="h-4 w-4 text-indigo-500 group-hover:text-white" />
                  </span>
                </div>
              </button>
            ))}
