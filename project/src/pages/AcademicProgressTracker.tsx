...
const AcademicProgressTracker: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      const testResultsSubscription = supabase
        .channel('academic_test_results_changes')
        .on(
          'postgres_changes',
          {
            event: '*',
            schema: 'public',
            table: 'academic_test_results',
  ...
  const metrics = useMemo(() => {
    ...
    const revisionConsistency = Math.min(100, recentActivity * 15); // 15% per activity, max 100%
  ...
  const ecoPoints = useMemo(() => {
    ...
    const revisionsDone = completedVideos.size; // Simplified: use video completions as revisions
  ...
  const weeklyInsights = useMemo(() => {
    ...
    if (weakSubjects.length > 0) {
      suggestedFocus.push(`Revise ${weakSubjects[0]} concepts`);
      suggestedFocus.push(`Complete more quizzes in ${weakSubjects[0]}`);
    }
    if (strongSubjects.length > 0) {
      suggestedFocus.push(`Maintain momentum in ${strongSubjects[0]}`);
    }
    suggestedFocus.push('Watch at least 2 video tutorials daily');
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-blue-900">Academic Progress Tracker</h1>
